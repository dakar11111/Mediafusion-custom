...
/// Cancel any currently active (downloading/queued) tasks so a new torrent
/// can be submitted immediately without hitting Seedr's single-slot queue limit.
...
async fn clear_active_tasks(
    ...
    let tasks = match list_tasks(http, token, forward).await {
        Ok(t) => t,
        Err(e) => {
            tracing::warn!("Seedr: could not list tasks before clearing queue: {e}");
            return;
        }
    };
    ...
    for task in &tasks {
        if task_is_downloading(task) {
            if let Some(id) = task["id"].as_i64() {
                tracing::info!("Seedr: cancelling active task {id} to free queue slot");
                api_delete(http, token, &format!("/tasks/{id}"), forward)
                    .await
                    .ok();
            }
        }
...
pub async fn get_video_url(
    ...
    let content_folder_id = match check_status(http, &token, &hash, forward).await? {
        ...
        TorrentStatus::NotFound => {
            // Cancel any active downloads to free the single-slot queue.
            clear_active_tasks(http, &token, forward).await;
